...
record_path = '/mnt/data/nas/gyl/RS_DATASET/boxes_jsonl/urur_record.jsonl'
boxes_path = '/mnt/data/nas/gyl/RS_DATASET/boxes_jsonl/urur_boxes.jsonl'
...
model = dict(
    ...
    backbone=dict(
        ...
        num_heads=[1, 2, 5, 8],
        num_layers=[3, 4, 6, 3]),
    decode_head=dict(type='Proto_SegformerHead', k=10, momentum=0.999, channels=1078, text_path=clip_text, in_channels=[64, 128, 320, 512], num_classes=8),
...
train_dataloader = dict(batch_size=2, num_workers=10, dataset=dict(pipeline=train_pipeline))
